...
(defun section-characteristics (bytes offset)
  "not finished, need to work out how to deal with multibit flags"
  (let ((characteristics
         (struct-value "Characteristics" (section-header bytes offset)))
        (codes '((IMAGE_SCN_TYPE_REG    #x00000000)
                 (IMAGE_SCN_TYPE_DSECT  #x00000001)
                 (IMAGE_SCN_TYPE_NOLOAD #x00000002)
                 (IMAGE_SCN_TYPE_GROUP  #x00000004)
                 (IMAGE_SCN_TYPE_NO_PAD #x00000008)
                 (IMAGE_SCN_TYPE_COPY   #x00000010)
    ...
    (dolist (c codes)
      (format t "~A ~a~% " (car c) (int-to-bin (last c))))
    ))
